controllers.controller('keepManagementCtrl', ["$scope", "ipService", "$modal", "loading", "errorModal", "confirmModal", "msgModal", function ($scope, ipService, $modal, loading, errorModal, confirmModal, msgModal) {
    $scope.ip_keep_list = [];
    $scope.selectData = [];
    ...
    $scope.searchList = function (currentPage = 1) {
        loading.open();
        $scope.is_submit = false
        ipService.search_keep_ips({}, $scope.filterObj, function (res) {
            loading.close();
            if (res.result) {
                $scope.ip_keep_list = res.data;
    ...
    $scope.getPagedDataAsync = function (pageSize, page) {
        $scope.setPagingData($scope.ip_keep_list ? $scope.ip_keep_list : [], pageSize, page);
    };
        ...
        columnDefs: [
            ...
            {field: "description", displayName: "描述"},
            {
                ...
                cellTemplate: '<div style="width:100%;text-align: center;padding-top:5px;">' +
                    ...
                    '<span class="label label-danger label-outline" ng-click="deleteItem(row)">删除</span>' +
                    '</div>'
            }
        ]
    };
    ...
    $scope.addIPs = function () {
        var modalInstance = $modal.open({
            templateUrl: static_url + 'client/views/ipManagement/addKeepIPs.html',
            windowClass: 'applyDialog',
            controller: 'addKeepCtrl',
            backdrop: 'static'
        });
        ...
        confirmModal.open({
            ...
            confirmClick: function () {
                ...
                ipService.delete_ips({
                    ...
                    loading.close();
                    if (res.result) {
                        angular.forEach($scope.ip_keep_list, function (j, i) {
                            if (j.ip == row.entity.ip && j.cloud_id == row.entity.cloud_id) {
                                $scope.ip_keep_list.splice(i, 1);
                            }
                        })
                        $scope.getPagedDataAsync($scope.pagingOptions.pageSize, $scope.pagingOptions.currentPage);
                    } else {
            ...
            if (res.result) {
                ...
                for (var i = 0; i < $scope.userList.length; i++) {
                    if ($scope.userList[i].text === $scope.selectItem.owner) {
                        $scope.selectItem.owner_id = String($scope.userList[i].id);
                        break;
                    }
                }
            } else {
    ...
    $scope.confirm = function () {
        for (var i = 0; i < $scope.userList.length; i++) {
            if ($scope.userList[i].id === parseInt($scope.selectItem.owner_id)) {
