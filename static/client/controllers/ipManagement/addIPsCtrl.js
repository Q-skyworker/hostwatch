controllers.controller("addIPsCtrl", function ($scope, $filter, loading, errorModal, $modalInstance, ipService, sysService, ipPoolService, msgModal) {
    $scope.title = "新增分配";
    ...
    $scope.confirm = function () {
        ...
        for (var i in $scope.userList) {
            if ($scope.userList[i].id == $scope.ipObj.owner) {
                $scope.ipObj.owner_name = $scope.userList[i].bk_username;
            }
        }
    ...
    var validate = function () {
        ...
        if ($scope.ipObj.ipType === "00") {
            ...
            if ($scope.ipObj.start_ip === "") {
                ...
                $scope.rules.start_ip = "起始IP不能为空！"
            } else if (!CWApp.isIP($scope.ipObj.start_ip)) {
                errors.push("起始IP格式不正确！");
                $scope.rules.start_ip = "起始IP格式不正确！";
            }
            if ($scope.ipObj.end_ip === "") {
                errors.push("结束IP不能为空！")
                $scope.rules.start_ip = "结束IP不能为空！"
            } else if (!CWApp.isIP($scope.ipObj.end_ip)) {
        ...
        } else if (!CWApp.isIP($scope.ipObj.gate_way)) {
            errors.push("网关格式有误!");
        ...
        } else if (!CWApp.isIP($scope.ipObj.dns)) {
