...
(function (window, $) {
    ...
    var ngFooter = function ($scope, grid) {
        ...
        $scope.changeMax = function () {
            if ($scope.pagingOptions.currentPage == null || $scope.pagingOptions.currentPage == undefined) {
    ...
    var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
        ...
        $scope.selectRowIndex = self.config.selectRowIndex;
        ...
        // $scope.selectRowIndex = [];
        $scope.allSelected = {is_checked: false};
        $scope.changeSelected = function (row) {
            var a = angular.copy(row.entity);

            a.grid_selected = undefined;
            var index = $scope.selectRowIndex.indexOf(row.rowIndex);
            if (index === -1) {
                if (row.entity.grid_selected) {
                    $scope.selectedItems.push(a);
                    $scope.selectRowIndex.push(row.rowIndex);
                }
            }
            else {
                if (!row.entity.grid_selected) {
                    $scope.selectedItems.splice(index, 1);
                    $scope.selectRowIndex.splice(index, 1);
                }
            }
            var is_checked = $scope.selectedItems.length === $scope.$eval(self.config.data).length;
            $scope.allSelected.is_checked = is_checked;
            if (is_checked)
                $("#kkk").addClass("checkbox-checked");
            else {
                $("#kkk").removeClass("checkbox-checked")
    ...
    var ngRowFactory = function (grid, $scope, domUtilityService, $templateCache, $utils) {
        ...
        self.rowConfig = {
            ...
            selectRowIndex: $scope.selectRowIndex,
    ...
    var ngSelectionProvider = function (grid, $scope, $parse) {
        ...
        self.selectRowIndex = grid.config.selectRowIndex;
                    ...
                    if (rows[rows.length - 1].beforeSelectionChange(rows, evt)) {
                        for (var i = 0; i < rows.length; i++) {
                            ...
                            if (index === -1) {
                                ...
                                self.selectRowIndex.push(ri.rowIndex);
                            }
                            else {
                                self.selectedItems.splice(index, 1);
                                self.selectRowIndex.splice(index, 1);
                            }
                        }
        ...
        self.toggleSelectAll = function (checkAll, bypass, selectFiltered) {
            ...
            if (bypass || grid.config.beforeSelectionChange(rows, checkAll)) {
                ...
                if (selectedlength > 0) {
                    ...
                    self.selectRowIndex.length = 0;
                }
                for (var i = 0; i < rows.length; i++) {
                    rows[i].entity.grid_selected = checkAll;
                    if (rows[i].clone) {
                        rows[i].clone.grid_selected = checkAll;
                    }
                    if (checkAll) {
                        var a = angular.copy(rows[i].entity);
                        a.grid_selected = undefined;
                        self.selectedItems.push(a);
                        self.selectRowIndex.push(rows[i].rowIndex);
                        $scope.allSelected.is_checked = true;
                    }
                    else {
                        $scope.allSelected.is_checked = false;
                    }
                        ...
                        return grid.init().then(function () {
                            ...
                            if (typeof options.data === "string") {
                                var dataWatcher = function (a) {
                                    ...
                                    $scope.selectRowIndex.splice(0, $scope.selectRowIndex.length);
                                    grid.data = $.extend([], a);
                                    grid.rowFactory.fixRowCache();
                                    angular.forEach(grid.data, function (item, j) {
                                        item.grid_selected = undefined;
    ...
    angular.module('ngGrid').run(['$templateCache', function ($templateCache) {
        ...
        $templateCache.put('checkboxCellTemplate.html',
            "<div class='ngSelectionCell checkbox'>" +
            "<label>" +
            "<span class=\"checkbox-span\" ng-class=\"{'checkbox-checked': row.entity.grid_selected}\">" +
            "<span class=\"checkbox-inner\"></span>" +
            "<input type=\"checkbox\" tabindex='-1' ng-model=\"row.entity.grid_selected\" ng-change='changeSelected(row)' class=\"ngSelectionCheckbox checkbox-input\">" +
            "</span>" +
            "</label>" +
            "</div>"
        ...
        // );
        $templateCache.put('checkboxHeaderTemplate.html',
            "<div ng-show=\"multiSelect\" class='ngSelectionHeader checkbox'>" +
            "<label>" +
            "<span id='kkk' class=\"checkbox-span\" ng-class=\"{'checkbox-checked': allSelected.is_checked}\">" +
            "<span class=\"checkbox-inner\"></span>" +
            "<input type=\"checkbox\" tabindex='-1' ng-model=\"allSelected.is_checked\" ng-change=\"toggleSelectAll(allSelected.is_checked, true)\" class=\"ngSelectionCheckbox checkbox-input\">" +
            "</span>" +
            "</label>" +
            "</div>"
        );
        ...
        $templateCache.put('footerTemplate.html',
            ...
            "\n" +
            "            <input class=\"ngPagerCurrent\" min=\"1\" max=\"{{currentMaxPages}}\" type=\"number\" style=\"width:50px; height: 24px; margin-top: 1px; padding: 0 4px;\" ng-change='changeMax()' ng-model=\"pagingOptions.currentPage\"/>\r" +
            "\n" +
